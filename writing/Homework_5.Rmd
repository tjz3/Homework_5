...
balt_homicidesv2 <- balt_homicidesv1 %>%
  ...
  str_remove_all(month_year, pattern = boundary(""))
...
```{r}
freddie <- tibble(month_year = "2015-04")
```
...
balt_homicide_plot <- balt_homicidesv3 %>%
  ...
  geom_vline(aes(xintercept = 201504, colour = "red"), show.legend = FALSE, 
             size = 2, linetype = 2) +
  scale_fill_manual(values = c("grey", "skyblue"), 
                    labels = c("Summer", "Winter")) +
  annotate(geom = "text", x = 201405, y = 43, label = "Arrest of Freddie Gray", color = "White") +
  theme_dark() +
  theme(legend.position = "bottom") +
  labs(title = "Homicides in Baltimore, MD", x = "Date", 
       y = "Monthly homicides", fill = "") +
  scale_x_continuous(breaks = 200800, 201000, 201200, 201400, 201600, 201800)
balt_homicide_plot

# need to fix x axis to be numeric then fix x axis scale and limits
# need to add geom_smooth to plot
```
